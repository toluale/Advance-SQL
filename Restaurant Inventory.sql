...
IS
restid INT :=-1; -- initially restaurant id is declared as -1 to avoid any err0rs. if -1 is set as default in case of no restaurant it directly writes no such restaurant from find_restaurant_id function
 BEGIN
 ...
 exception 
  when others THEN 
   dbms_output.put_line ('waiter could not be inserted'); -- handling the err0rs
 END;
/
...
--insert data into the Customers_T table (13 individual customers)
INSERT INTO Customers_T VALUES (seq_customers.nextval, 'Cust1', 'kmacneil@gmail.com', '543 NE. WINTergreen Rd', 'Jacksonville Beach', 'MD', 21045, 4485446110928919);
INSERT INTO Customers_T VALUES (seq_customers.nextval, 'Cust11', 'zsvensson@verizon.net', '788 Valley Dr.', 'Anaheim', 'MD', 21045, 5456984657180975);
INSERT INTO Customers_T VALUES (seq_customers.nextval, 'Cust3', 'rwang@outlook.com', '326 Country Ave', 'Elizabethtown', 'MD', 21046, 6011846746604610);
INSERT INTO Customers_T VALUES (seq_customers.nextval, 'Cust111', 'mewart@hotmail.com', '244 Miller Dr', 'Middleburg', 'MD', 21045, 340005289560995);
...
INSERT INTO Customers_T VALUES (seq_customers.nextval, 'CustNY2', 'kmacneil@gmail.com', '7942 Miramar Rd', 'San Diego', 'NY', 10045, 5242547177568836);
INSERT INTO Customers_T VALUES (seq_customers.nextval, 'CustNY3', 'zsvensson@verizon.net', '2801 Market St', 'Pittsburgh', 'NY', 10045, 5598377348641431);
...
END;
/
--Executing menu update quantity procedures
...
BEGIN
dbms_output.put_line('State'||'   '||'Total Tip Amount');-- handling the output. This statement is used to display the output in an organized manner.
open c1;
loop
-- Fetching the data from cursor c1 INTo local variables total_tip and state_name
...
BEGIN
	...
	close c1;
	if counter = 0 then
		dbms_output.put_line('No Menu Items Found');
	end if;
END;
/
...
AS
    ...
    cuisType cuisine_type_t.cuisinename%type;
    menu menu_items_t.menuname%type;
    i INT;
    total INT;
...
Cursor c1 is SELECT count(orders_t.menuid), menuname, cuisinename --cursor within the PROCEDURE
    ...
    where cuisinename = cuisineOrder and --condition for the input parameter
          orders_t.menuid = menu_items_t.menuid and
          menu_items_t.cuisineid = cuisine_type_t.cuisineid 
    group by menuname, cuisinename
    order by count(orders_t.menuid) desc;
BEGIN
    i := 1;
    total := 0;
...
Loop --creating a loop to iterate through the tables
    ...
    exit when i>2 or c1%notfound; --exit condition
    -- prINT out statement
    dbms_output.put_line(menutype || ' of ' || cuisineType || ' cuisine was ordered ' || menuCount || ' time(s).');
    END loop;
    Exception
    When no_data_found then
    DBMS_output.put_line('No orders for this cuisine type exist in the database.');
    Close c1;
End;    
...
as
	res_Name restaurant_t.Rest_name%type;
	res_id restaurant_t.Rest_ID%type;
	sumpaid orders_t.amount_paid%type;
	i int :=0;
	cursor c1 is SELECT restaurant_t.rest_name, restaurant_t.Rest_ID, sum(orders_t.amount_paid)
	from restaurant_t, orders_t 
    where restaurant_t.Rest_State = reState 
	AND orders_t.Rest_ID = restaurant_t.Rest_ID
	group by restaurant_t.rest_name, restaurant_t.Rest_ID
	ORDER BY sum(orders_t.amount_paid) DESC;
begin
	open c1;
	loop
	Fetch c1 into res_Name,res_id, sumpaid;
	i:= i+1;
	exit when i >= 3 or c1%notfound;
	dbms_output.put_line(res_Name ||' in ' || reState || ' has a total income of $' || sumpaid);
	end loop;
close c1;
	Exception
    When no_data_found then
    DBMS_output.put_line('There are no transactions for restaurants in this state.');
end;
/
...
DECLARE
    counter int :=0; --utilizing a counter to address the EXCEPTION: What if there are no customer entries?
    ...
    ORDER BY sum(amount_paid) DESC; --the key difference from the BOTTOM Spender Report
...
BEGIN
    ...
    EXIT WHEN i > 4 OR top_spenders%notfound;
...
DECLARE
    ...
    WHERE customers_t.Customer_ID = orders_t.Customer_ID
    GROUP BY customer_name
    ORDER BY sum(amount_paid); --the default sort is ASCENDING
    custName customers_t.Customer_Name%TYPE;
    amt_paid orders_t.Amount_Paid%TYPE;
    i INT;
    total INT;
BEGIN
    ...
    EXIT WHEN i > 4 OR bottom_spenders%notfound;
        dbms_output.put_line(custName); 
    counter := counter + 1;
    END LOOP;
